...
export class FilePublisher {
  ...
  getFallBackLogFileName(topic, logType) {
    switch (logType) {
      case 'fallback':
        return `${this.getFallBackLogFileNamePrefix(topic)}.fallback.log`
      case 'orphanedAck':
        return `${this.getFallBackLogFileNamePrefix(topic)}.orphanedAck.fallback.log`
      case 'largeMesg':
        return `${this.getFallBackLogFileNamePrefix(topic)}.largeMesg.fallback.log`
      default:
        log.error(`Error not a valid logType, expected fallback | orphanedAck | largeMesg, was ${logType}`)
        return `${this.getFallBackLogFileNamePrefix(topic)}.FIXME.fallback.log`
    }
  }

  getFallBackLogFileNamePrefix(topic) {
    const yyyymmddhh = moment.utc().format('YYYYMMDDhh')
    return `${this.options.directory}/${topic}~${yyyymmddhh}~${this.options.instanceId}~${this.stats.readySeq}`
  }
  ...
  async publish(mesg) {
    ...
    this.publishInternal(this.getFallBackLogFileName(mesg.topic, 'fallback'), mesg)
  }

  // mesg = {topic, ackKey}
  async publishOrphanedAck(mesg) {
    if (!this.options.enabled) return
    this.publishInternal(this.getFallBackLogFileName(mesg.topic, 'orphanedAck'), mesg)
  }

  // mesg = {topic, mesgValue}
  async publishMesgTooLarge(mesg) {
    if (!this.options.enabled) return
    this.publishInternal(this.getFallBackLogFileName(mesg.topic, 'largeMesg'), mesg)
  }
