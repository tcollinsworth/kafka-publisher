...
import partitioner from 'murmur2-partitioner'

import { init as initLogger } from './logging.mjs'
import { FilePublisher, initLogger as initFilePublisherLogger } from './file-publisher.mjs'
import { validateKey } from './validate-key.mjs'
import { validateValue } from './validate-value.mjs'

const instanceId = uuidV4() // used for creating unique fallback files per instance

let log

// node-rdkafka (librdkafka) murmur2 keyed partitioner does not produce the same partitions as kafka java and no-kafka
...
export class KafkaPublisher {
  ...
  async init() {
    if (this.initExecuting) {
      ...
      if (log.isLevelEnabled('debug')) log.debug(mesg)
  ...
  async startKafkaConnectRetryProcess() {
    ...
    await promiseRetry(async (retry) => {
      if (this.connExecuting) {
        if (log.isLevelEnabled('debug')) log.debug('startKafkaConnectRetryProcess currently executing, skipping')
        return
      }
      try {
        if (log.isLevelEnabled('debug')) log.debug('startKafkaConnectRetryProcess trying to connect')
        this.connExecuting = true
        this.consecutiveKafkaErrorCnt = 0
        log.info('startKafkaConnectRetryProcess')
        await this.disconnect()
        this.createKafkaProducer()
        log.info('kafkaProducer start connecting')
        ...
        if (log.isLevelEnabled('debug')) log.debug('startKafkaConnectRetryProcess finally')
  ...
  // await callback to gracefully complete or return/throw error (not connected) or timeout
  async disconnect() {
    if (this.kafkaProducer == null) return Promise.resolve()
    return new Promise((resolve) => {
      ...
      try {
        timeout = setTimeout(() => {
          ...
          log.error('kafka disconnect timeout')
        }, 5000)
        ...
        log.error(err, 'kafka disconnect error')
      }
    })
  }
  ...
  async queueInternal(message, cb) {
    ...
    try {
      if (!this.kafkaReady) {
        this.startKafkaConnectRetryProcess() // in case it's not running
        this.fallback.publish({ topic, mesgValue, ackKey })
      ...
      const queued = this.kafkaProducer.produce(
        topic,
        this.getPartition(key, topic), // partition
  ...
  getPartition(key, topic) {
    let partitionCnt = topicPartitions[topic]
    if (partitionCnt == null && this.kafkaProducer != null && this.kafkaProducer._metadata != null) {
      const topicMetaData = this.kafkaProducer._metadata.topics.find((metaData) => metaData.name === topic)
      if (topicMetaData === null) return null
      partitionCnt = topicMetaData.partitions.length
      topicPartitions[topic] = partitionCnt
    }
    if (partitionCnt == null) return null
    const partition = partitioner.partition(key, partitionCnt)
    return partition
  }
  ...
  queueMesgAckTimeout(topic, mesgValue, key, ackKey, cb) {
    const queuedMesg = {
      ...
      cb: {
        cb, // function - the cb for awaiting completion
        settled: false, // cb does not have a state variable, used to avoid race with mesg timeout
      },
      timoutObj: null, // timeout object that will handle timeout if delivery-report ack not received, cancelled on receive
      settleAndRemove: null, // function - clear timeout, set settled true, remove from pending, if error, cb(error), else cb(null, deliveryReport)
    }
    ...
    queuedMesg.timoutObj = setTimeout((qdMesg) => {
      try {
        ...
        this.fallback.publish({ topic, mesgValue, ackKey: qdMesg.ackKey })
    ...
    queuedMesg.settleAndRemove = (error, deliveryReport, isMesgTooLarge) => {
      ...
      if (error == null) {
        ...
        if (isMesgTooLarge) {
          this.fallback.publishMesgTooLarge({ topic, mesgValue: queuedMesg.mesgValue, ackKey: queuedMesg.ackKey })
        } else {
          this.fallback.publish({ topic, mesgValue: queuedMesg.mesgValue, ackKey: queuedMesg.ackKey })
        }
  ...
  deliveryReport(error, deliveryReport) {
    try {
      if (error != null) {
        log.error(error, `DeliveryReport error ${stringify(deliveryReport)}`)
        this.incrementConsecutiveKafkaErrorCnt()
        if (this.pending() < 1) {
      ...
      if (error == null) {
        // there was a positive ack, even if it timedout and is dicovered missing/orphaned below
        // clear the consecutiveKafkaErrorCnt to prevent unnecesarily reconnecting
        this.consecutiveKafkaErrorCnt = 0
      }
      ...
      if (qdMesg == null) {
        ...
        this.fallback.publishOrphanedAck({ topic: deliveryReport.topic, ackKey: deliveryReport.opaque })
        return
      }
