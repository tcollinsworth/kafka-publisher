...
export class KafkaPublisher {
  ...
  async handleQueuedInternal() {
    console.log('************** handleQueuedInternal', this.q)
    ...
    try {
      ...
      while (this.q.length > 0) {
        ...
        console.log('************** handleQueuedInternal fallback.send')
        // eslint-disable-next-line no-await-in-loop
  ...
  async publishInternal(topic, key, value) {
    console.log('************** publishInternal')
    let mesgKey
    try {
      try {
        mesgKey = validateKey(key)
        validateValue(value)
      } catch (err) {
      ...
      const kafkaMesg = { topic, message: { key: mesgKey, value: stringify(value) } }
...
console.log('************** publishInternal returning', this.kafkaReady)
  ...
  async kafkaSendBlocksWhileDownInternal(kafkaMesg) {
    console.log('************** kafkaSendBlocksWhileDownInternal')
