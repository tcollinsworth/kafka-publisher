...
// TODO console.log(RdKafka.CODES.ERRORS.ERR_MSG_SIZE_TOO_LARGE)
...
export class KafkaPublisher {
   ...
   * if cb == null promise is returned which is resolved when ack received from kafka or rejected if error or timeout
   */
  ...
  queueInternal(message, cb) {
    ...
    try {
      ...
      // TODO clear ack timeout promise, write to fallback and ?start recovery
      const error = new Error(`produce queue error, writing to fallback, msg ${this.stats.msgCnt}, ${err.message}`)
  ...
  queueMsgAckTimeout(topic, msgValue, key, ackKey, cb) {
    ...
    queuedMsg.timoutObj = setTimeout((qdMsg) => {
      ...
      // TODO write to fallback, and ?start recovery
      log.warn('timeout, write to fallback, msg %d', qdMsg.ackKey)
