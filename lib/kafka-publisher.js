  ...
  async publish(topic, key, value) {
    ...
    try {
      try {
        ...
        this.updateErrorStats(new Error(`Error validating message key or value ${err.message}`))
        return true
      }

      // kafkaProducer.send retries for about 3 seconds with default configs
      // TODO / FIXME if kafka down, awaits here forever
      ...
      if (resp == null || resp.length < 1 || resp[0].error != null) {
        ...
        if (resp != null && resp.length > 0 && resp[0].error != null && resp[0].error.code === 'MessageSizeTooLarge') {
          ...
          this.updateErrorStats(new Error('Kafka MessageSizeTooLarge (default > 1 MB), discarding'))
          return true
        }
        // start background process to keep retrying until kafka recovers
        if (this.kafkaReady) setImmediate(this.kafkaBackroundRetry.bind(this), topic)
        this.kafkaReady = false

        log.error(`Error publishing to kafka ${stringify(resp)}, ${stringify(kafkaMesg)}`)
        this.updateErrorStats(new Error(`Error publishing to kafka ${resp[0].error.code}`))
        return false
      }
      return true
    } catch (err) {
      log.error(`Error publishing to kafka ${err.message}, ${stringify(kafkaMesg)}`, err)
      this.updateErrorStats(new Error(`Error publishing to kafka ${err.message}`))
      return false
    }
  }

  async kafkaBackroundRetry(topic) {
    // eslint-disable-next-line no-plusplus
    ++this.stats.backgroundRetries
    try {
      this.kafkaReady = await this.publish(topic, 'test', { test: true })
      if (this.kafkaReady) log.info('Kafka recovered, switching from file append fallback, manually load fallback messages and delete file')
    } catch (err) {
      log.error(`Error kafkaBackroundRetries topic:${topic} ${err.message}`, err)
      this.updateErrorStats(new Error(`Error kafkaBackroundRetries topi:${topic} ${err.message}`))
    } finally {
      // retry ever 10 seconds, logging every time
      if (!this.kafkaReady) setTimeout(this.kafkaBackroundRetry.bind(this), 10000, topic)
    }
  }
