...
export class KafkaPublisher {
  ...
  queue(key, message, topic = this.options.defaultTopic) {
    if (topic == null) {
  ...
  async handleQueued() {
    ...
    try {
      ...
      while (this.q.length > 0) {
        ...
        if (this.kafkaReady) {
          if (this.publish(mesg.topic, mesg.key, mesg.value)) {
            // eslint-disable-next-line no-plusplus
            ++this.stats.sentCnt
  ...
  publish(topic, key, value) {
    ...
    try {
      ...
      const resp = this.kafkaProducer.send(kafkaMesg)
      ...
      if (resp == null || resp.length < 1 || resp[0].error != null) {
        ...
        if (resp != null && resp.length > 0 && resp[0].error.code === 'MessageSizeTooLarge') {
