...
// TODO should it start the recovery logic or a timeout to start the recovery logic if not kafkaReady before timeout
   ...
   * a promise is returned which is resolved when ack received from kafka or rejected if error or timeout
   *
   * cb args (error, deliveryReport)
   * <pre>
   ...
   * </pre>
   */
  async queue(key, value, topic = this.options.defaultTopic, cb) {
  ...
  async queueInternal(message, cb) {
    ...
    if (topic == null) {
      ...
      return this.handleQueueError(err, cb)
    }
    ...
    try {
      ...
      if (!this.kafkaReady) {
        ...
        return this.handleQueueError(new Error(`kafka NOT ready, writing to fallback, msg ${this.stats.msgCnt}`), cb)
      }
      ...
      if (err.code != null && err.code === RdKafka.CODES.ERRORS.ERR_MSG_SIZE_TOO_LARGE) {
        // TODO messageTooLarge should not incr consecutiveKafkaErrorCnt
      ...
      return this.handleQueueError(error, cb)
    }
  }

  // eslint-disable-next-line class-methods-use-this
  handleQueueError(err, cb) {
    if (cb != null) {
      cb(err)
    }
    return Promise.reject(err)
  ...
  queueMsgAckTimeout(topic, msgValue, key, ackKey, cb) {
    const queuedMsg = {
      ...
      settleAndRemove: null, // clear timeout, set settled true, remove from pending, if error, reject promise, else resolve promise, call cb, return promise
    }
