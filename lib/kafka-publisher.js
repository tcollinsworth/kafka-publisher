...
export class KafkaPublisher {
  ...
  async publish(topic, key, value) {
    ...
    try {
      ...
      this.kafkaReady = await timeout(this.kafkaSendBlocksWhileDown(kafkaMesg), 3000)
  ...
  async kafkaSendBlocksWhileDown(kafkaMesg) {
    try {
      ...
      if (resp == null || resp.length < 1 || resp[0].error != null) {
        ...
        if (!this.backgroundRetrying) setImmediate(this.kafkaBackroundRetry.bind(this), kafkaMesg.topic)
        return false
      }
      return true
    } catch (err) {
  ...
  async kafkaBackroundRetry(topic) {
    ...
    try {
      ...
      if (!this.kafkaPending && !this.kafkaReady) setTimeout(this.kafkaBackroundRetry.bind(this), 10000, topic)
    }
  }
