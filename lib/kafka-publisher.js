...
export class KafkaPublisher {
  ...
  async queueInternal(message) {
    ...
    try {
      ...
      if (!this.kafkaReady) {
        ...
        return Promise.reject(new Error(`kafka NOT ready, writing to fallback, mesg: ${this.stats.mesgCnt}`))
      }
      const queuedMesg = this.queueMesgAckTimeout(topic, mesgValue, key, ackKey)
      ...
      if (!queued) {
        ...
        return queuedMesg.settleAndRemove(error)
      }
      this.stats.queueCnt += 1
      return queuedMesg.promise.promise
    } catch (err) {
  ...
  queueMesgAckTimeout(topic, mesgValue, key, ackKey) {
    const queuedMesg = {
    ...
    queuedMesg.promise.promise = new Promise((resolve, reject) => {
      queuedMesg.promise.resolve = resolve
      queuedMesg.promise.reject = reject
    })
    ...
    queuedMesg.settleAndRemove = (error, deliveryReport) => {
      if (queuedMesg.promise.settled) return queuedMesg.promise.promise
      clearTimeout(queuedMesg.timoutObj)
      queuedMesg.promise.settled = true
      delete this.q[queuedMesg.ackKey]
      if (error == null) {
        this.stats.ackSuccessCnt += 1
        queuedMesg.promise.resolve(deliveryReport)
      } else {
        this.consecutiveKafkaErrorCnt = 0
