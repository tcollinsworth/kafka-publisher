...
export class KafkaPublisher {
  ...
  async init() {
    await promiseRetry(async (retry) => {
      try {
        await this.kafkaProducer.init() // new NoKafka.Producer().init()
  ...
  // either queues to kafka or to fallback (filesystem) if kafka not ready
  ...
  async publishInternal(topic, key, value) {
    try {
      const kafkaMesg = { topic, message: { key, value: stringify(value) } }
  ...
  // tries to send to kafka, but will not return if awaiting client connect/reconnect
  // be aware the pending message may dupe on recovery or be lost if the system restarts before being saved to fallback
  async kafkaSendBlocksWhileDownInternal(kafkaMesg) {
    try {
      ...
      const resp = await this.kafkaProducer.send(kafkaMesg, { codec: NoKafka.COMPRESSION_GZIP })

      if (debug.enabled) log.debug('kafka resp:', stringify(resp), ', kafka mesg:', stringify(kafkaMesg))
      ...
      if (resp == null || resp.length < 1 || resp[0].error != null) {
        ...
        this.updateErrorStatsInternal(new Error(`Error publishing to kafka ${this.getKafkaErrorCode(resp)}`))
        this.kafkaReady = false
  ...
  getKafkaErrorCode(resp) {
    if (resp != null && resp.length > 0 && resp[0].error != null) {
      return resp[0].error.code
    }
    log.error(`unknownError-FIXME resp ${stringify(resp)}`)
    return 'unknownError-FIXME'
  }

  // periodically retries to publish a test message to kafka until it succeeds, then it sets kafkaReady
  // to switch back from publishing to fallback (file) to publishing directly to kafka
  async kafkaBackroundRetryInternal(topic) {
    ...
    try {
      ...
      if (this.kafkaReady) {
        ...
        log.info('Kafka recovered, continue publishing to kafka')
        this.handleQueuedInternal()
      }
    } catch (err) {
