...
export class FilePublisher {
  constructor(options) {
    ...
    // TODO file per topic, new file every day and kafka (reconnect) ready
    this.options.fallbackLog = `${this.options.directory}/fallback-${this.options.instanceId}.log`
  ...
  async publishInternal(mesg) {
    this.stats.mesgCnt += 1

    // TODO topic in filename so processable per topic
    // TODO new file hourly yyyymmddhh, small sortable
    // TODO new file on each ready
    // TODO filename fallback-topic-yyyymmddhh-instanceUuid-instanceReadySeq.log
    await promiseRetry(async (retry) => {
      try {
        if (debug.enabled) log.debug('FilePublisher.send %s', mesg)
  ...
  // TODO implement log rolling, i.e., start of every kafka error window or daily
  ...
  async attempCreateDirInternal(dir) {
    try {
      mkdirp.sync(dir)
      log.warn(`Created fallback directory ${dir}, if ephemperal, i.e., Docker, on crash or restart messages will be lost forever`)
    } catch (err) {
